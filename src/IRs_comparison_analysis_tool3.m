...
function output_folder = IRs_comparison_analysis_tool3(measured_wav_file, simulated_wav_file)
    close all;
    clc;  % Remove 'clear' to keep input parameters
   ...
   % === LOAD SIGNALS ===
    fprintf('\nLoading provided signals...\n');
    
    % Load recorded signal from provided file
    if nargin < 2
        error('Both measured_wav_file and simulated_wav_file must be provided');
    end
    
    if ~exist(measured_wav_file, 'file')
        error('Measured WAV file not found: %s', measured_wav_file);
    end
    
    if ~exist(simulated_wav_file, 'file')
        error('Simulated WAV file not found: %s', simulated_wav_file);
    end
    
    [recorded_signal, fs_rec] = audioread(measured_wav_file);
    [simulated_signal, fs_sim] = audioread(simulated_wav_file);
    
    % Convert to mono if needed
    if size(recorded_signal, 2) > 1
        recorded_signal = mean(recorded_signal, 2);
    end
    if size(simulated_signal, 2) > 1
        simulated_signal = mean(simulated_signal, 2);
    end
    
    fprintf('  ✓ Loaded measured: %s (%.2f s, %.0f Hz)\n', measured_wav_file, length(recorded_signal)/fs_rec, fs_rec);
    fprintf('  ✓ Loaded simulated: %s (%.2f s, %.0f Hz)\n', simulated_wav_file, length(simulated_signal)/fs_sim, fs_sim);
    ...
    % Create output directory early so we can save WAV files there
    timestamp_str = datestr(now, 'yyyymmdd_HHMMSS');
    output_folder = sprintf('IR_comparison_results_%s', timestamp_str);
    if ~exist(output_folder, 'dir')
        mkdir(output_folder);
    end
    
    % Save WAV files in the results folder
    audiowrite(fullfile(output_folder, 'template_trimmed_recorded.wav'), trimmed_recorded, target_fs);
    audiowrite(fullfile(output_folder, 'template_trimmed_simulated.wav'), trimmed_simulated, target_fs);
    audiowrite(fullfile(output_folder, 'reference_template.wav'), template, target_fs);
    
    fprintf('  ✓ %s/template_trimmed_recorded.wav\n', output_folder);
    fprintf('  ✓ %s/template_trimmed_simulated.wav\n', output_folder);
    fprintf('  ✓ %s/reference_template.wav (for reference)\n', output_folder);
    ...
    results_filename = fullfile(output_folder, sprintf('IR_comparison_results_%s.mat', timestamp_str));
    ...
    % Generate console report
    generateConsoleReport(comparison_results, output_folder);
    
    % Save figures
    saveFigures(output_folder, timestamp_str);
...
function saveFigures(output_folder, timestamp_str)
    % Save all open figures
    fig_handles = findall(0, 'Type', 'figure');
    
    for i = 1:length(fig_handles)
        fig = fig_handles(i);
        if ~isempty(fig.Name) && ~contains(fig.Name, 'Advanced Acoustic Analysis')
            try
                % Check if figure has axes with content
                axes_handles = findall(fig, 'Type', 'axes');
                has_content = false;
                
                for j = 1:length(axes_handles)
                    children = get(axes_handles(j), 'Children');
                    if ~isempty(children)
                        has_content = true;
                        break;
                    end
                ...
                if has_content
                    ...
                    fprintf('  ⚠ Skipping empty figure: %s\n', fig.Name);
                end
                
            catch ME
                fprintf('  ⚠ Warning: Could not save figure %s: %s\n', fig.Name, ME.message);
            end
        end
    end
end
...
function generateConsoleReport(results, output_folder)
